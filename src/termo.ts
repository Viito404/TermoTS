export class Termo{
  ...
  palavraSecretaSemAcento:string = "";
  rodadas:number = 0;
  ...
  gerarPalavraAleatoria(){
   let palavras: string[] = ["ácido", "adiar", "ímpar", "algar", "amado", "amigo", "anexo", "anuir", "aonde", "apelo",
   ...
   let numeroAletorio: number = 0;
   numeroAletorio = Math.floor(Math.random() * palavras.length);
   this.palavraSecreta = palavras[numeroAletorio].toUpperCase();
  ...
  jogadorGanhou(palavra:string):boolean{
     return palavra === this.palavraSecretaSemAcento;
  }
  jogadorPerdeu():boolean{
    return this.rodadas === 5;
  }
}